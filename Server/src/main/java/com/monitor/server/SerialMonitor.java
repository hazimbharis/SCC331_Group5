...
public class SerialMonitor {
    ...
    private smartCup cup = new smartCup();
    private ArrayList<LocationTracker> trackers = new ArrayList<>();
    public String z1light ="";
    public String z2light ="";
    public String z3light = "";
    ...
    *   This should add the required dependency  */
        ...
        microbit.addDataListener(new SerialPortMessageListener() {
            ...
            @Override
            public byte[] getMessageDelimiter() { return new byte[] { System.getProperty("line.separator").getBytes()[0]}; }
            ...
            public void serialEvent(SerialPortEvent event)
            {
                ...
                if(types.length<6){
                    ...
                    if(types[0].trim().equals("Chair")){
                        setMD("chair");
                    } else if (types[0].trim().equals("Switch")) {
                        setMD("switch");
                    } else if (types[0].trim().equals("Door")) {
                        setMD("door");
                    }else{
                        ...
                        if (channel == 0) {
                            ...
                            String check = types[1].split(":")[0]; //cup
                            System.out.println(check);
                            System.out.println(check.equals("Percent"));
                            if(check.equals("Tilt")){
                                int tilt = Integer.parseInt((types[1].split(":")[1].trim()));
                                cup.setTilt(tilt);

                            }else if(check.equals("Percent")) {
                                int percent = Integer.parseInt(types[1].split(":")[1].trim());
                                cup.setPercent(percent);
    ...
    public void updateTwitter(){
        try{
            Runtime.getRuntime().exec("cmd.exe /c start C:/Users/veter/Documents/Uni/softwareEngineering/year3/330/330Group5/Server/src/main/java/com/monitor/server/tweet.bat");
        }catch (Exception e){if(DEBUG){e.printStackTrace();}}

    }
    public void updateLights(int zone){
        try{
            if(zone==1){
                byte[] msg = z1light.getBytes();
                microbit.writeBytes(msg,msg.length);
            } else if (zone==2) {
                byte[] msg = z2light.getBytes();
                microbit.writeBytes(msg,msg.length);
            } else if (zone==3) {
                byte[] msg = z2light.getBytes();
                microbit.writeBytes(msg,msg.length);
            ...
            System.out.println("no microbit?");
        }
    ...
    public void setPing(int zone){
        try{
            ...
            if(DEBUG){System.out.println(getURL);System.out.println(responseCode);}
    ...
    public smartCup getCup(){return cup;}
}